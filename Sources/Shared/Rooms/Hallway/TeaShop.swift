    ...
    static func make() -> ItemDefinition {
        ItemDefinition(
            ...
            fullDescriptionProvider: { state in
                let coins = coins(in: state)
                if coins > 0 {
                    return "Перед тобой стойка чайного бизнеса. Пока что примитивная, но рабочая. Заработано: \(coins) монет. Цены: кипяток — \(mugPrice) монет, чай — \(teaPrice) монет, сладкий чай — \(sweetTeaPrice) монет."
                }
                return "Перед тобой стойка чайного бизнеса. Пока что примитивная, но рабочая. Цены: кипяток — \(mugPrice) монет, чай — \(teaPrice) монет, сладкий чай — \(sweetTeaPrice) монет. Пакетики чая и сахар продаются в продуктовом."
            },
