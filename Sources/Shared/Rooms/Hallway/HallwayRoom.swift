...
enum HallwayRoom {
    ...
    static func make() -> RoomDefinition {
        ...
        let bedroomDoor = DoorDefinition(id: "hallway.door.bedroom", name: "дверь в спальню", targetRoomID: .bedroom, targetRoomPosition: GridPosition(x: 1, y: 1), state: .closed, focusNodeID: "hallway.node.bedroomDoor", shortPrompt: "Рядом дверь в спальню.", openResultText: "Ты открыл дверь и прошел в спальню.", lockedText: "Заперто.", sound: .doorCloseHallway)
        let lockedDoor = DoorDefinition(id: "hallway.door.storage", name: "дверь в кладовку", targetRoomID: .hallway, targetRoomPosition: nil, state: .locked, focusNodeID: "hallway.node.storageDoor", shortPrompt: "Рядом дверь в кладовку.", openResultText: "", lockedText: "Заперто.", sound: .doorCloseHallway)
        let neighborDoor = DoorDefinition(id: neighborDoorID, name: "входная дверь", targetRoomID: .hallway, targetRoomPosition: nil, state: .closed, focusNodeID: neighborDoorNodeID, shortPrompt: "У самой двери кто-то зло переминается и ждет, когда ты откроешь.", openResultText: "", lockedText: "Сейчас она не открывается.", sound: .doorCloseHallway)
